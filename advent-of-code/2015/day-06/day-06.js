...
const toggleLights = (instructions) => {
  ...
  instructions.split('\n').forEach(instruction => {
    ...
    if (instruction[0] === 'turn') {
      const start = instruction[2].split(',').map(Number);
      const end = instruction[4].split(',').map(Number);
      for (let r = start[0]; r <= end[0]; r++) {
        for (let c = start[1]; c <= end[1]; c++) {
          if (instruction[1] === 'on') grid[r][c] = 1;
          if (instruction[1] === 'off') grid[r][c] = 0;
        }
      }
    }
    
    if (instruction[0] === 'toggle') {
      const start = instruction[1].split(',').map(Number);
      const end = instruction[3].split(',').map(Number);
      for (let r = start[0]; r <= end[0]; r++) {
        for (let c = start[1]; c <= end[1]; c++) {
...
const toggleLights2 = (instructions) => {
  const grid = createGrid(1000, 1000);
  ...
  instructions.split('\n').forEach(instruction => {
    ...
    if (instruction[0] === 'turn') {
      ...
      for (let r = start[0]; r <= end[0]; r++) {
        for (let c = start[1]; c <= end[1]; c++) {
          if (instruction[1] === 'on') grid[r][c]++;
          if (instruction[1] === 'off' && grid[r][c] > 0) grid[r][c]--;
        }
      }
    }
...
const answer1 = toggleLights(input);
const answer2 = toggleLights2(input);
console.log(`expected ${answer1} to equal 569999:`, answer1 === 569999);
console.log(`expected ${answer2} to equal 17836115:`, answer2 === 17836115);
