...
// Check to see if any bots have two chips
const botsWithTwoChips = () => {
  const result = [];
  
  for (let num in bots) {
    if (bots[num].values.length === 2) {
      result.push(num);
    }
  }
...
const passChips = (botNum) => {
  // Sort chip values
  bots[botNum].values = bots[botNum].values.sort((a,b) => a-b);

  // Check if bot has microchips values we're looking for   
  if (bots[botNum].values[0] === microchips[0] && bots[botNum].values[1] === microchips[1]) {
    console.log(`${botNum} is responsible for comparing microchips ${bots[botNum].values}`);
  }

  // Bots to pass chips to
  const botLow = bots[botNum].low;
  const botHigh = bots[botNum].high;
  
  // If bot to pass to doesn't exist, create it
  if (!bots[botLow]) {
    bots[botLow] = {values: []};
  }
  ...
  // Pass chips to other bots
  bots[botLow].values.push(bots[botNum].values.shift());
  bots[botHigh].values.push(bots[botNum].values.shift());
};

const botSort = (instructions, chip1, chip2) => {
  instructions = instructions.split('|').forEach(instruction => {
    (instruction.split(' ')[0] === 'value') ? assignValues.push(instruction) : assignBots.push(instruction);
  });
  
  microchips = [chip1, chip2].sort((a,b) => a-b);
  
  // Set up bot assignments
  assignBots.forEach(bot => {
    bot = bot.split(' ');
    const botNum = `${bot[0]} ${bot[1]}`;
  ...
  assignValues.forEach(bot => {
    bot = bot.split(' ');
    const botNum = `${bot[4]} ${bot[5]}`;
    bots[botNum].values.push(+bot[1]);

    let twoChipBots = botsWithTwoChips();    
    while(twoChipBots.length > 0) {
      twoChipBots.forEach(num => {
        passChips(num);
      });
      
      twoChipBots = botsWithTwoChips();
    }
  });

  const outputTotal = bots['output 0'].values[0] * bots['output 1'].values[0] * bots['output 2'].values[0];
  console.log(`The product of outputs 0, 1 and 2 is ${outputTotal}.`);
};

botSort(givenInstructions, 61, 17);
