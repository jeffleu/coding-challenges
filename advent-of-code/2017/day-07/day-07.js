class Node {
  constructor(id, parent = null, weight = 0, children = []) {
    this.id = id;
    this.weight = weight;
  ...
  getTotalWeight() {
    if (!this.children.length) return this.weight;
    return this.children.reduce((output, child) => output + child.getTotalWeight(), this.weight);
  }
}

const createNodeMapping = (input) => {
  const nodeMapping = {};
  ...
  input = input.split('\n')
    ...
    .forEach(row => {
      const parent = row[0];
      const weight = Number(row[1].slice(1, row[1].length - 1));
      const children = row.slice(3).map(child => child.replace(/,/, ''));

      // Create Node for parent
      if (!nodeMapping.hasOwnProperty(parent)) {
        nodeMapping[parent] = new Node(parent, null, weight, children);
      } else {
        nodeMapping[parent].addWeight(weight);
        nodeMapping[parent].addChildren(children);
      }

      // Create Node for children
      children.forEach(child => {
        if (!nodeMapping.hasOwnProperty(child)) {
          nodeMapping[child] = new Node(child, parent);
        } else {
          nodeMapping[child].addParent(parent);
        }
      });
    });
  
  // Map all children to instances of Node class
  for (let node in nodeMapping) {
    const children = nodeMapping[node].children.map(child => nodeMapping[child]);
    nodeMapping[node].addChildren(children);
  }
  
  return nodeMapping;
};

const findBottomProgram = (input) => {
  const nodeMapping = createNodeMapping(input);
  let current = nodeMapping[Object.keys(nodeMapping)[0]];
  while (current.parent) current = nodeMapping[current.parent];
  return current.id;
};

const findUnbalanced = (input) => {
  const nodeMapping = createNodeMapping(input);
  let current = nodeMapping[findBottomProgram(input)];
  let childrenWeights = current.children.map(child => child.getTotalWeight());
  let difference;
  
  while (childrenWeights.length && !childrenWeights.every(n => n === childrenWeights[0])) {
    const weightCount = childrenWeights.reduce((output, n, i) => {
      !output.hasOwnProperty(n) ? output[n] = {count: 1, index: i} : output[n].count++;
      return output;
    }, {});

    for (let weight in weightCount) {
      const {count, index} = weightCount[weight];
      if (count === 1) {
        if (!difference) {
          difference = Object.keys(weightCount).map(Number).reduce((output, n) => {
            return output === 0 ? n : Math.abs(output - n);
          }, 0);
        }
        current = current.children[index];
        childrenWeights = current.children.map(child => child.getTotalWeight());
      }
    }
  }
  
  return current.weight - difference;
};
...
console.log('bottom program:', findBottomProgram(input));
console.log('unbalanced program:', findUnbalanced(input));
