// Part 1
const corruptionChecksum = (spreadsheet) => {
  let result = 0;
  
  const rows = spreadsheet
    .split('\n')
    .map(row => row.split(' ').map(Number).sort((a, b) => a - b))
    .forEach(row => result += row[row.length - 1] - row[0]);
  
  return result;
};

// Part 2
const corruptionChecksum2 = (spreadsheet) => {
  let result = 0;
  
  const rows = spreadsheet
    .split('\n')
    .map(row => row.split(' ').map(Number).sort((a, b) => b - a))
    .forEach(row => {
...
const input = `1208 412 743 57 1097 53 71 1029 719 133 258 69 1104 373 367 365
...
const answer1 = corruptionChecksum(input);
const answer2 = corruptionChecksum2(input);
console.log(`expected ${answer1} to equal 54426:`, answer1 === 54426);
console.log(`expected ${answer2} to equal 333:`, answer2 === 333);
