const parseInput = (input) => {
  return input.split('\n').map(instruction => {
    ...
    return {
      ...
      amt: Number(instruction[2]),
      regCheck: instruction[4],
      operator: instruction[5],
      amtCheck: Number(instruction[6]),
    };
  });
};

const performOperation = (registers, register, regCheck, operator, amtCheck, incDec, amt) => {
  if (operator === '<' && registers[regCheck] < amtCheck) {
    incDec === 'inc' ? registers[register] += amt : registers[register] -= amt;
  }
...
const getMaxFromRegister = (registers) => {
  return Object.values(registers).reduce((output, n) => Math.max(output, n), -Infinity);
};

const getLargestValue = (instructions) => {
  instructions = parseInput(instructions);
  const registers = {};
  let max = 0;
  
  for (let i = 0; i < instructions.length; i++) {    
    const {register, incDec, amt, regCheck, operator, amtCheck} = instructions[i];
    if (!registers.hasOwnProperty(register)) registers[register] = 0;
    if (!registers.hasOwnProperty(regCheck)) registers[regCheck] = 0;
    performOperation(registers, register, regCheck, operator, amtCheck, incDec, amt);
    max = Math.max(max, getMaxFromRegister(registers));
  }
  
  return {part1: getMaxFromRegister(registers), part2: max};
};
...
const answer1 = getLargestValue(input).part1;
const answer2 = getLargestValue(input).part2;
console.log(`expected ${answer1} to equal 4448:`, answer1 === 4448);
console.log(`expected ${answer2} to equal 6582:`, answer2 === 6582);
