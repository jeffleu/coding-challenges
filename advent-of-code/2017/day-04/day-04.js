const isValidPassphrase = (passphrase, part) => {
  if (part === 1) passphrase = passphrase.split(' ');
  if (part === 2) passphrase = passphrase.split(' ').map(p => p.split(' ').sort().join(''));
  const set = new Set();

  for (let i = 0; i < passphrase.length; i++) {
    if (!set.has(passphrase[i])) {
      set.add(passphrase[i]);
    } else {
      return false;
    }
  }
...
const countValidPassphrases = (passphrases, part) => {
  let result = 0;

  passphrases.split('\n').forEach(passphrase => {
    if (isValidPassphrase(passphrase, part)) result++;
  });
...
const answer1 = countValidPassphrases(input, 1);
const answer2 = countValidPassphrases(input, 2);
console.log(`expected ${answer1} to equal 477:`, answer1 === 477);
console.log(`expected ${answer2} to equal 167:`, answer2 === 167);
