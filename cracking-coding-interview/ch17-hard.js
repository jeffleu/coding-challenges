...
/************************************************************************
  ...
  17.2: SHUFFLE
	...
	shuffle-in other words, each of the 52! permutations of the deck has to
...
/************************************************************************
  ...
  17.4: MISSING NUMBER
	...
	in A with a single operation. The elements of A are represented in
	binary, and the only operation we can use to access them is "fetch the
...
/************************************************************************
  ...
  17.7: BABY NAMES
	...
	only problem with this is that some names have multiple spellings.

	For example, "John" and "Jon" are essentially the same name but would
	be listed separately in the list. Given two lists, one of names or
	frequencies and the other of pairs of equivalent names, write an
	algorithm to print a new list of the true frequency of each name. Note
	that if John and Jon are synonyms, and Jon and Johnny are synonyms,
	then John and Johnny are synonyms. (It is both transitive and
...
/************************************************************************
  ...
  17.8: CIRCUS TOWER
	...
	her. Given the heights and weights of each person in the circus, write
...
/************************************************************************
  ...
  17.10: MAJORITY ELEMENT
	...
	element. If there is no majority element, return -1. Do this inO(N)
	time and 0(1) space.
...
/************************************************************************
  ...
  17.12: BINODE
	...
	linked list (where nodel is the previous node and node2 is the next
	node). Implement a method to convert a binary search tree (implemented
	with BiNode) into a doubly linked list. The values should be kept in
	order and the operation should be performed in place (that is, on the
...
/************************************************************************
  ...
  17.13: RE-SPACE
	...
	and capitalization later; right now you need to re-insert the spaces.
...
/************************************************************************
  ...
  17.16: THE MASSEUSE
	...
	requests and is debating which ones to accept. She needs a 15-minute
	break between appointments and therefore she cannot accept any adjacent
	requests. Given a sequence of back-to-back appointment requests (all
	multiples of 15 minutes, none overlap, and none can be moved), find the
...
/************************************************************************
  ...
  17.17: MULTI SEARCH
	...
	search b for each small string in T.
...
/************************************************************************
  ...
  17.18: SHORTEST SUPERSEQUENCE
	...
	one longer. Find the shortest subarray in the longer array that
	contains all the elements in the shorter array. The items can appear in
	any order.
...
/************************************************************************
  ...
  17.19: MISSING TWO
	...
	exactly once, except for one number that is missing. How can you find
	the missing number in O(N) time and 0(1) space? What if there were two
	numbers missing? 
...
/************************************************************************
  ...
  17.21: VOLUME OF HISTOGRAM
	...
	(Refer to page 200 of PDF for image.)
...
/************************************************************************
  ...
  17.26: SPARSE SIMILARITY
	...
	The similarity of two documents (each with distinct words) is defined
	to be the size of the intersection divided by the size of the union.

	For example, if the documents consist of integers, the similarity of
	{ 1, 5, 3} and { 1, 7, 2, 3} is 0. 4, because the intersection has size 
	2 and the union has size 5.

	We have a long list of documents (with distinct values and each with an
	associated ID) where the similarity is believed to be "sparse:'That is,
	any two arbitrarily selected documents are very likely to have
	similarity 0. Design an algorithm that returns a list of pairs of
	document IDs and the associated similarity.

	Print only the pairs with similarity greater than 0. Empty documents
	should not be printed at all. For simplicity, you may assume each
	document is represented as an array of distinct integers.
	...
	Hints: #484, #498, #510, #518, #534, #547, #555, #561, #569, #577,
				 #584, #603, #611, #636
